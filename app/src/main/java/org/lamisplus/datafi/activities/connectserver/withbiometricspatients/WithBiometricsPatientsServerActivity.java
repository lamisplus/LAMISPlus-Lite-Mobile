...
public class WithBiometricsPatientsServerActivity extends LamisBaseActivity {

    public WithBiometricsPatientsServerContract.Presenter mPresenter;
    public WithBiometricsPatientsServerFragment withBiometricsPatientsServerFragment;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_find_patient_with_biometrics_server);

        withBiometricsPatientsServerFragment = (WithBiometricsPatientsServerFragment) getSupportFragmentManager().findFragmentById(R.id.findPatientServerContentFrame);
        if (withBiometricsPatientsServerFragment == null) {
            withBiometricsPatientsServerFragment = WithBiometricsPatientsServerFragment.newInstance();
        }
        if (!withBiometricsPatientsServerFragment.isActive()) {
            addFragmentToActivity(getSupportFragmentManager(), withBiometricsPatientsServerFragment, R.id.findPatientServerContentFrame);
        }
        ...
        if (queryBundle != null) {
            ...
            mPresenter = new WithBiometricsPatientsServerPresenter(withBiometricsPatientsServerFragment, query);
        }else{
            mPresenter = new WithBiometricsPatientsServerPresenter(withBiometricsPatientsServerFragment);
        }
